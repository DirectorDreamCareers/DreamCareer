...
function HeroSection({divClass=''}) {
  return (
    <div className={divClass} >
      <Slider {...settings}>
          {images.map(image => (
            <div key={image.alt} className='relative bg-slate-600'>
              <img key={image.alt} src={image.img} className='object-contain mx-auto w-full bg-slate-600 opacity-80' alt={image.alt} />
